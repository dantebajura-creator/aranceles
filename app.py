...
class EscoClient:
    ...
    def get_grupos_aranceles_bursatiles(self) -> Tuple[bool, str, Optional[List[Dict[str, Any]]]]:
        ok, msg = self.ensure_token()
        if not ok:
            return False, msg, None

        url = self._endpoint("/api/v9/get-grupos-aranceles-bursatiles")
        r = requests.get(url, headers=self._headers(accept_json=True, send_json=False), timeout=30)
        if r.status_code >= 400:
            return False, f"get-grupos-aranceles-bursatiles falló ({r.status_code}): {r.text[:300]}", None

        try:
            data = r.json()
            # Swagger muestra una lista de objetos {codigo, descripcion}
            if isinstance(data, list):
                return True, "OK", data
            # a veces puede venir envuelto
            if isinstance(data, dict) and "value" in data and isinstance(data["value"], list):
                return True, "OK", data["value"]
            return True, "OK", data  # best-effort
        except Exception:
...
# --------- Dynamic grupos (opcional) ---------
GRUPOS_MAP_DYNAMIC: Dict[int, str] = {}

def refresh_grupos_map_dynamic() -> None:
    """Intenta traer el mapeo (codigo -> descripcion) desde ESCO.
    Si falla, no rompe nada (se mantiene el mapeo hardcodeado).
    """
    global GRUPOS_MAP_DYNAMIC
    ok, _msg, data = esco.get_grupos_aranceles_bursatiles()
    ...
    if isinstance(data, list):
        for it in data:
            if isinstance(it, dict):
                c = it.get("codigo") if "codigo" in it else it.get("Codigo")
                d = it.get("descripcion") if "descripcion" in it else it.get("Descripcion")
                try:
                    ci = int(c)
                except Exception:
                    continue
    ...
    if request.method == "POST" and cuenta:
        ...
        if not ok:
            ...
            def walk(obj):
                ...
                if isinstance(obj, dict):
                    for k, v in obj.items():
                        if str(k).lower() in ("codgrupoaroperburs", "cod_grupo_ar_oper_burs", "grupoaroperburs"):
                            try:
                                grupo_code = int(v)
                                return
                            except Exception:
            ...
            if grupo_code is None:
                flash("No encontré 'codGrupoArOperBurs' en la respuesta de get-detalle-cuenta.", "warning")
            else:
                grupo_desc = GRUPOS_MAP.get(grupo_code)
                if not grupo_desc:
                    # intento dinámico (sin romper nada si falla)
                    if not GRUPOS_MAP_DYNAMIC:
                        refresh_grupos_map_dynamic()
                    grupo_desc = GRUPOS_MAP_DYNAMIC.get(grupo_code)
                if not grupo_desc:
                    grupo_desc = f"Grupo código {grupo_code}"
